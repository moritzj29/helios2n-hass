...
Per the 2N HTTP API spec, InputChanged/OutputChanged log events carry
...
@pytest.mark.parametrize(
    ("port_identifier", "known", "expected"),
    [
        # Exact string match — the common case (e.g. "led_secured")
        ("input1", {"input1", "relay1"}, "input1"),
        ("relay1", {"input1", "relay1"}, "relay1"),
        ("led_secured", {"led_secured", "input1"}, "led_secured"),
        # Case/whitespace differences are tolerated defensively
        ("INPUT1", {"input1", "relay1"}, "input1"),
        (" input1 ", {"input1", "relay1"}, "input1"),
    ],
)
def test_resolve_port_id_from_event(port_identifier, known, expected):
    """Event port string should resolve to the matching coordinator cache key."""
    assert _resolve_port_id_from_event(port_identifier, known) == expected
