...
from .coordinator import Helios2nMappingDataUpdateCoordinator, Helios2nPortDataUpdateCoordinator, Helios2nSwitchDataUpdateCoordinator, Helios2nSensorDataUpdateCoordinator
...
async def _update_switch_state_from_log_event(
    ...
    if not isinstance(lock_data, dict):
        return
    coordinator: Helios2nSwitchDataUpdateCoordinator | Any | None = lock_data.get("coordinator")
...
async def _update_port_state_from_log_event(
    ...
    if not isinstance(switch_data, dict):
        return
    coordinator: Helios2nPortDataUpdateCoordinator | Any | None = switch_data.get("coordinator")
    if coordinator is None or not hasattr(coordinator, "async_set_updated_data"):
        return
