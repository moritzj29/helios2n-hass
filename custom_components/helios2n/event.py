...
class Helios2nUserAuthenticatedEventEntity(EventEntity):
    """Event entity for UserAuthenticated log events."""

    _attr_has_entity_name = True
    _attr_event_types = ["authenticated"]
    _attr_entity_registry_enabled_default = True

    def __init__(self, entry_id: str, device: Py2NDevice):
        self._entry_id = entry_id
        self._device = device
        self._attr_unique_id = f"{self._device.data.serial}_user_authenticated"
        self._attr_name = "User Authenticated"
    ...
    def _handle_log_event(self, event: Mapping[str, Any]) -> None:
        ...
        auth_method = extracted.get("auth_method")
        ...
        if auth_method is not None:
            attributes["auth_method"] = auth_method
        if "originator" in payload:
            attributes["originator"] = payload["originator"]
        if "utcTime" in event:
            attributes["utc_time"] = event["utcTime"]

        self._trigger_event("authenticated", attributes)
        if self.hass is not None:
            self.async_write_ha_state()
